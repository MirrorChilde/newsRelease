<template>
  <div>
    <div class="news">
      ...
      <div class="newsList">
        <el-table :data="newsList" style="width: 100%" :header-cell-style="{ background: '#E8EBEF' }">
          <el-table-column label="发布时间" width="200" sortable prop="date">
            <template slot-scope="scope">
              <i class="el-icon-time"></i>
              <span style="margin-left: 10px">{{ scope.row.updateTime.substring(0, 10) +
                "" + scope.row.updateTime.substr(11, 5) }}</span>
            </template>
          </el-table-column>
          <el-table-column label="新闻名称" width="380">
            <template slot-scope="scope">
              <span style="margin-left: 10px">{{ scope.row.title }}</span>
            </template>
          </el-table-column>
          <el-table-column label="新闻栏目" width="120">
            <template slot-scope="scope">
              <span style="margin-left: 10px">{{ scope.row.plateName }}</span>
            </template>
          </el-table-column>
          <el-table-column ref="filterTable" prop="tag" label="状态" width="120" :formatter="formatter"
            ...
            <template slot-scope="scope">
              <el-tag :type="scope.row.status == '0' ? 'primary' : (scope.row.status == '3' ? 'danger' : 'success')"
                disable-transitions>{{ scope.row.status == '0' ? '审核中' : (scope.row.status == '3' ? '审核不通过' :
                  '审核成功') }}</el-tag>
            </template>
          </el-table-column>
      ...
      <div class="block" style="margin-top: 15px;">
        <el-pagination small @size-change="getNewsList" @current-change="getNewsList" :current-page.sync="current"
